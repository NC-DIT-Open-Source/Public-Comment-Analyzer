...
The operator log emits `inference_budget_threshold` warnings once when persistent application reservations cross 80%, 90%, or 100% of the deployment call or cost cap. Events include only the resource, threshold, and deployment scope; they contain no comment text, credentials, model identifiers, or actual billing claims. Configure your host's log monitoring to route these events into alerts. The app sends no external notifications. Failed reservations do not generate repeated threshold warnings, and the existing hard caps still reject requests that would exceed the limits.
